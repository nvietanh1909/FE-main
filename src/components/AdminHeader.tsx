...
export default function AdminHeader() {
    ...
    return (
        <header 
            className="bg-white h-[5.3rem] flex items-center justify-center font-roboto"
                                    ...
                                    <div className="absolute right-0 top-full w-64 bg-white rounded-lg shadow-2xl border-gray-200 py-2 z-50" style={{ boxShadow: '0 -4px 6px -1px rgba(0, 0, 0, 0.1), 0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
                                        ...
                                        <div className="px-4 py-3 border-b border-gray-100 pb-6 border-b-solid">
                                            <div className="flex items-center gap-3 ">
                                                ...
                                                <div>
                                                    <p className="text-[1rem] text-gray-900 m-0">{user?.name || 'Admin'}</p>
                                                    <div className="flex items-center gap-2">
                                                        <p className="text-gray-500 m-0 text-[0.8rem]">@{user?.username || 'admin'}</p>
                                                        <p className=" m-0 text-[0.8rem] px-2 bg-blue-200 text-blue-800 rounded-full">{user?.role || 'Quản trị viên'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
